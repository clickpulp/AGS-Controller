...
int ControllerScriptManagedObject::Dispose(const char* address, bool force)
{
...
int ControllerScriptManagedObject::Serialize(const char* address, char* buffer, int bufsize)
{
	// put 1 byte there
	...
	: ControllerModule(engine)
{
	...
	engine->AddManagedObjectReader(constructname, m_managedObjectReader);
}
